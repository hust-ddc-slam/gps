...
GPS::GPS(std::string dev) : portName_(std::move(dev)), nh_("~"), initial_set_(false) {
    ...
    traj = nh_.advertise<geometry_msgs::PoseStamped>("gps_traj",20);
    try {
...
void GPS::process(void) {
    ...
    while (true) {    
        ...
        while (std::getline(is, core_data)){
                 ...
                 gps_pose.header.frame_id = "traj";
                 gps_pose.pose.position.x = enup[0];
                 gps_pose.pose.position.y = enup[1];
                 gps_pose.pose.position.z = enup[2];
                 gps_pose.pose.orientation.x = 0;
                 gps_pose.pose.orientation.y = 0;
                 gps_pose.pose.orientation.z = 0;
                 gps_pose.pose.orientation.w = 1;
                 traj.publish(gps_pose);
                 std::ofstream outfile;
                 std::string filename;
                 filename = "/home/larry/lidar_ws/data/" + str + ".txt";
